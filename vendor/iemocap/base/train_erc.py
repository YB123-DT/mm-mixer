...
def train_modality_model(
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        print(f"[{modality_name}] Epoch {epoch:3d}/{num_epochs} | "
        ...
        if val_f1 > best_f1:
            ...
            counter += 1
            if counter >= patience:
                print(f"[{modality_name}] Early stopping at epoch {epoch}")
                break

    if best_weights is not None:
        model.load_state_dict(best_weights)

    print(f"[{modality_name}] Phase 1 done. Best val F1: {best_f1:.4f}")
...
def train_fusion_model(
    ...
    best_f1, patience, counter, best_weights = 0.0, early_stopping_patience, 0, None
    best_epoch = 0
    lr_reduce_counter = 0
    ...
    if metrics["test_f1"]:
        best_test_idx = int(np.argmax(metrics["test_f1"]))
        metrics["best_test_epoch"] = best_test_idx + 1
        metrics["best_test_f1"] = metrics["test_f1"][best_test_idx]
        metrics["best_test_acc"] = metrics["test_acc"][best_test_idx]
...
def train_full_pipeline(
    ...
    if skip_pretrain:
        print("=== Skipping Phase 1 encoder pretrain (skip_pretrain=true) ===", flush=True)
    else:
        print("=== Phase 1: Encoder Pretrain ===", flush=True)

    if "t" in modalities and not skip_pretrain:
    ...
    if "a" in modalities and not skip_pretrain:
    ...
    if "v" in modalities and not skip_pretrain:
    ...
    fusion_model = HierarchicalAttentionFusion(
        ...
        dropout=cfg.get("fusion_dropout", cfg.get("dropout_rate", 0.15)),
        ...
        use_dialogue_graph=cfg.get("use_dialogue_graph", False),
        dialogue_graph_modalities=cfg.get("dialogue_graph_modalities", []),
        dialogue_graph_window=cfg.get("dialogue_graph_window", 8),
        dialogue_graph_layers=cfg.get("dialogue_graph_layers", 2),
        dialogue_graph_dropout=cfg.get("dialogue_graph_dropout", 0.1),
        dialogue_graph_num_speakers=cfg.get("dialogue_graph_num_speakers", 2),
    ).to(device)
    ...
    fusion_criterion = MultitaskFusionLoss(
        ...
        focal_gamma=cfg.get("focal_reweight_gamma", 2.5),
        aux_loss_type=cfg.get("aux_loss_type", "poly"),
        main_loss_type=cfg.get("main_loss_type", "poly"),
        per_sample_focal=cfg.get("per_sample_focal", False),
    ...
    fusion_model, fusion_metrics = train_fusion_model(
        ...
        num_epochs,
        device,
        early_stopping_patience=cfg.get("fusion_early_stopping_patience", 12),
        early_stopping_threshold=cfg.get("fusion_early_stopping_threshold", 0.001),
        lr_reduce_patience=cfg.get("fusion_lr_reduce_patience", 3),
        lr_reduce_factor=cfg.get("fusion_lr_reduce_factor", 0.5),
    )
    ...
    return fusion_model, fusion_report, fusion_metrics
...
if __name__ == "__main__":
    ...
    if fixed.get("use_dialogue_graph", False):
        dg_modalities = "".join(fixed.get("dialogue_graph_modalities", [])) or "none"
        exp_name += f"_dlggraph-{dg_modalities}"
    ...
    base = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "Dataset", "Data")
    train_csv = cfg_all.get("train_csv", os.path.join(base, "train_iemo.csv"))
    val_csv = cfg_all.get("val_csv", os.path.join(base, "dev_iemo.csv"))
    test_csv = cfg_all.get("test_csv", os.path.join(base, "test_iemo.csv"))
    ...
    train_ds = IEMODataset(
        ...
        embed_dims,
        is_training=True,
        dialogue_graph=fixed.get("use_dialogue_graph", False),
        dialogue_graph_modalities=fixed.get("dialogue_graph_modalities", []),
    )
    val_ds = IEMODataset(
        val_df,
        val_features.get("v", {}),
    ...
    fusion_model, fusion_report, fusion_metrics = train_full_pipeline(
        train_loader,
        val_loader,
        test_loader,
        device,
    ...
    print("====================================")
    print(
        f"[diagnostic] best_epoch={fusion_metrics.get('best_epoch', 0)} "
